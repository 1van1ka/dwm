...
static const Rule rules[] = {
	{ .monitor = -1, .wintype = WTYPE "DIALOG", .isfloating = 1 },
	{ .monitor = -1, .wintype = WTYPE "UTILITY", .isfloating = 1 },
	{ .monitor = -1, .wintype = WTYPE "TOOLBAR", .isfloating = 1 },
	{ .monitor = -1, .wintype = WTYPE "SPLASH", .isfloating = 1 },
	{ .monitor = -1, .class = "feh", .isfloating = 1, .iscentered = 1 },
	{ .monitor = -1, .class = "Viewnior", .isfloating = 1, .iscentered = 1 },
	{ .monitor = -1, .class = "Pcmanfm", .isfloating = 1, .iscentered = 1, .height = 400, .width = 900 },
	{ .monitor = -1, .class = "pavucontrol", .isfloating = 1, .width = 750, .height = 400 },
	{ .monitor = -1, .class = "firefox", .tags = 1 << 0 },
	{ .monitor = -1, .class = "zen", .tags = 1 << 0 },
	{ .monitor = -1, .class = "TelegramDesktop", .tags = 1 << 3 },
	{ .monitor = -1, .class = "discord", .tags = 1 << 3 },
	{ .monitor = -1, .class = "steam", .tags = 1 << 2 },
	{ .monitor = -1, .class = "Blueman-manager", .isfloating = 1 },
	{ .monitor = -1, .class = "mpv", .isfloating = 1, .iscentered = 1, .tags = 1 << 4, .height = 800, .width = 1300 },
	{ .monitor = -1, .class = "resources", .isfloating = 1, .iscentered = 1, .height = 800, .width = 1400 },
	{ .monitor = -1, .class = "spterm",	.tags = SPTAG(0),	.isfloating = 1, .height = 500, .width = 1000, .y = 100 }
};
...
static const BarRule barrules[] = {
	...
	{ -1,        0,     BAR_ALIGN_CENTER, width_awesomebar,         draw_awesomebar,        click_awesomebar,        NULL,                    "awesomebar" },
};

static const Layout layouts[] = {
	{ .symbol = "[]=",  .arrange = tile,           .border = {1, 0},       .gaps = {1, 0} },    /* first entry is default */
	{ .symbol = "><>",  .arrange = NULL,           .border = {1, 1},       .gaps = {0, 0} },    /* no layout */
	{ .symbol = "[M]",  .arrange = monocle,        .border = {0, 0},       .gaps = {0, 0} },
	{ .symbol = "|M|",  .arrange = centeredmaster, .border = {1, 0},       .gaps = {1, 1} },
	{ .symbol = " D ",  .arrange = deck,           .border = {1, 1},       .gaps = {1, 1} }
};

static const char *ipcsockpath = "/tmp/dwm.sock";
static IPCCommand ipccommands[] = {
	{ .name = "focusmon",   .func = {focusmon},   .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_SINT} },
	{ .name = "focusstack", .func = {focusstack}, .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_SINT} },
	{ .name = "incnmaster", .func = {incnmaster}, .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_SINT} },
	{ .name = "killclient", .func = {killclient}, .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_SINT} },
	{ .name = "quit",       .func = {quit},       .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_SINT, ARG_TYPE_NONE} },
	{ .name = "setmfact",   .func = {setmfact},   .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_FLOAT} },
	{ .name = "setstatus",  .func = {setstatus},  .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_STR} },
	{ .name = "tag",        .func = {tag},        .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_UINT} },
	{ .name = "tagmon",     .func = {tagmon},     .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_UINT} },
	{ .name = "togglebar",  .func = {togglebar},  .argc = 1,   .arg_types = (ArgType[]){ARG_TYPE_NONE} }
};
...
static const char *const autostart[] = {
  ...
  "dunst", NULL,
  "redshift", NULL,
  "remind", "-z", "-k'notify-send -a \"Remind|Напоминание\" -t 15000 \"$(echo \\\"%s\\\" | sed \\\"s/\\\\\\\\\\//g\\\")\"'", "/home/ivanika/.reminders", NULL,
	NULL
};

static const Key keys[] = {
	/* modifier                     key            function                argument */
	{ .mod = MODKEY,           .keysym = XK_Escape,    .func = spawn,       .arg = {.v = (const char*[]){ TERM, "--class", "resources", "-e", "btop", NULL } } },
	...
	{ MODKEY,                 			XK_o,          spawn,           		   {.v = (const char*[]){ "pavucontrol-qt", NULL } } },
