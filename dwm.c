  ...
  for (i = 0; i < LENGTH(rules); i++) {
    ...
    if ((!r->title || strstr(c->name, r->title)) &&
      ...
      if (r->isfloating) {
        ...
        if (r->tags & SPTAGMASK) {
          c->x = r->x > 0 ? r->x : c->mon->wx + (c->mon->ww / 2 - WIDTH(c) / 2);
          c->y =
...
void drawbarwin(Bar *bar) {
  ...
  int rx, lx, rw,
      lw; // bar size, split between left and right if a center module is
          // added
  const BarRule *br;
...
void focusviadmenu(const Arg *arg) {
  Client *c, *match = NULL;
  Monitor *m;
  FILE *fp;
  char selected[512];
  ...
  snprintf(dmenu_cmd, sizeof(dmenu_cmd), "echo \"");
  ...
  for (m = mons; m; m = m->next) {
    for (c = m->clients; c; c = c->next) {
      ...
      snprintf(win_info, sizeof(win_info), "%d.%d [0x%lx] %s -> %s%c",
               m->num + 1, ffs(c->tags), c->win, class,
      ...
      size_t info_len = strlen(win_info);
      if (len + info_len < sizeof(dmenu_cmd) - 64) {
        strcat(dmenu_cmd, win_info);
        len += info_len;
      }
    }
  }
  ...
  if (fgets(selected, sizeof(selected), fp)) {
    ...
    int mon_num = -1, tag_num = -1;
    char id_buf[32] = {0};
    char class_buf[256] = {0};
    char name_buf[256] = {0};
    unsigned long selected_winid = 0;

    if (sscanf(selected, "%d.%d [%31[^]]] %255[^-] -> %255[^\n]", &mon_num,
               &tag_num, id_buf, class_buf, name_buf) >= 4) {
      mon_num--;

      if (strncmp(id_buf, "0x", 2) == 0) {
        selected_winid = strtoul(id_buf, NULL, 16);
      }
      ...
      if (tag_num > 0 && tag_num <= NUMTAGS) {
        view(&(Arg){.ui = 1 << (tag_num - 1)});
      }

      for (m = mons; m; m = m->next) {
        for (c = m->clients; c; c = c->next) {
          if (c->win == selected_winid) {
            match = c;
            break;
          }
        }
